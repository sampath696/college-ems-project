...
if(isset($_REQUEST['did'])){
	...
	$sql = "DELETE FROM tblenquiry where id = $did";
	$query = $dbh->prepare($sql);
	$query->execute();
...
{
	?><td>Read</td>
                                    <td><a href="manage-enquires.php?did=<?php echo htmlentities($result->id);?>"
                                            onclick="return confirm('Do you really want to remove from enquires')">Delete
                                        </a>
                                    </td>
                                    <?php } else {?>
